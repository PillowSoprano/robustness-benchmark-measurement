...
C
      ...
      INTEGER IDV
      COMMON/DVEC/IDV(20)
C     SEVERITY PATCH: DSEV(20) scales each IDV disturbance amplitude.
C     DSEV = 1 reproduces the original code exactly.
      DOUBLE PRECISION DSEV
 ...
 500  CONTINUE
  ...
  950 CONTINUE
      END IF
C     SEVERITY PATCH: DSEV factors on the step-disturbance amplitudes.
      XST(1,4)=TESUB8(1,TIME)-IDV(1)*DSEV(1)*0.03D0
     .-IDV(2)*DSEV(2)*2.43719D-3
      XST(2,4)=TESUB8(2,TIME)+IDV(2)*DSEV(2)*0.005D0
      XST(3,4)=1.D0-XST(1,4)-XST(2,4)
      TST(1)=TESUB8(3,TIME)+IDV(3)*DSEV(3)*5.D0
      TST(4)=TESUB8(4,TIME)
      TCWR=TESUB8(5,TIME)+IDV(4)*DSEV(4)*5.D0
      TCWS=TESUB8(6,TIME)+IDV(5)*DSEV(5)*5.D0
      R1F=TESUB8(7,TIME)
      R2F=TESUB8(8,TIME)
      DO 1010 I=1,3
      UCVR(I)=YY(I)
...
C     SEVERITY PATCH: DSEV factors on the feed-loss faults.
      FTM(3)=VPOS(3)*(1.D0-IDV(6)*DSEV(6))*VRNG(3)/100.0
      FTM(4)=VPOS(4)*(1.D0-IDV(7)*DSEV(7)*0.2D0)
     .*VRNG(4)/100.0+1.D-10
 ...
 9010 CONTINUE
     ...
     .(TCWS-TWS)-QUS*1.D6/1.8)/HWS
      IVST(10)=IDV(14)
      IVST(11)=IDV(15)
C     severity patch: valve-sticking dead band scales with DSEV
      VST(10)=2.0D0*DSEV(14)
      VST(11)=2.0D0*DSEV(15)
      IVST(5)=IDV(19)
      IVST(7)=IDV(19)
      IVST(8)=IDV(19)
      IVST(9)=IDV(19)
